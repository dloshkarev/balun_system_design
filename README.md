...
- Disks_for_iops (только холодные данные) = `(0.1*69444) / 1000 = 7`
- Результат: 1.2 * 7 = **9 SSD дисков 50 Тб**

Расчеты для метаданных постов и пользователей:
- Disks_for_capacity = `(2 + 47) / 100ТБ = 1`
- Disks_for_throughput = `(35 + 3) / 500 Мб/с = 1`
- Disks_for_iops (только холодные данные) = `(33 + 0.1*17526 + 2315) / 1000 = 5`
...
### Партицирование, шардирование, репликация

**Партицирование**

Партицировать скорее всего есть смысл только те сущности, к которым наибольшая нагрузка по чтению, в нашем случае - посты и комментарии.

Плюсы:
- ускорение запросов на чтение
- удобный перенос данных в "холодные"
- удобнее обслуживание БД в перспективе, если мы например решим что комментарии и посты имеют срок жизни

Наиболее подходящим вариантом выглядит **Range based по дате создания** - можем например делать партиции по дням или неделям, тогда будет проще формировать ленту постов и комментариев

**Шардирование**

- В нашем случае шардирование обязательно требуется только для таблицы с комментариями, для остальных сущностей объем не выглядит избыточным.
- Оптимальным выглядит **шардирование с хэшированием id поста**, к которому относятся комментарии, тогда все комментарии одного поста будут находиться на одном физическом сервере, а именно так они и запрашиваются
- Функцию роутинга по шардам предлагаю передать отдельному proxy-серверу
- Так как RPS на чтение комментариев велик, то предлагаю не ограничиваться одним proxy-сервером, а выделить несколько под эту задачу и для роутинга уже между ними использовать DNS

**Репликация**

Так как наш сервис является гео распределенным по своей сути, то нам требуется:
- Распределить данные по нескольким датацентрам (репликам), расположенным в разных частях СНГ
- Данные пользователей, постов и реакций реплицируются полностью в каждой реплике
- Для комментариев реплицируем шарды и распределяем их по ДЦ так, чтобы полный набор данных можно было собрать исключив любой ДЦ из списка

Такое решение позволит:
- Увеличить отказоустойчивость и надежность системы
- Обеспечить заявленную latency на загрузку постов и комментариев

Для хранения фото ситуация нестандартная из-за большого объема, поэтому я вижу следующие варианты:
1. Реплицировать только фото для ленты, тогда нам будет нужно в разы меньше дисков и хостов соответственно. Минус - репутационные потери в случае утери оригиналов фото, хотя их можно снизить оперативно заменив оригинал фото на фото из ленты (с потерей качества разумеется). Вывод - вариант выглядит технически привлекательным, но предположим что бизнес против
2. Реплицировать только оригиналы фото, тогда мы сможем сэкономить немного (~10) дисков, а в случае аварии сделать фото для ленты из оригиналов, и временно используя те же оригиналы как фото для ленты. Вывод - вариант требует слишком много манипуляций для такой небольшой выгоды 
3. Реплицировать все

Итоги:
- Останавливаемся на варианте №3, как наиболее надежном. В итоге реплицируем как холодные, так и горячие данные
- Тип репликации:
  - По топологии - предлагаю master-master между разными ДЦ и master-slave внутри одного ДЦ
  - По механизму синхронизации - асинхронную, т.к у нас нет критических данных, а требования по производительности серьезные
  - По уровню детализации - для репликации master-master видимо нужна логическая репликация, т.к ее проще реализовать. А в случае master-slave - подойдет физическая, для производительности

**Хосты**

Берем фактор репликации = 2 и по 2 диска на хост. В итоге эти показатели взаимоисключаются в расчете

- Посты и пользователи = 6 SSD дисков 50 Тб = 6 хостов, если учитывать снижение iops нагрузки, то достаточно **3 хостов**
- Комментарии = 9 SSD дисков 50 Тб = 9 хостов, если учитывать снижение iops нагрузки, то достаточно **5 хостов**
- Кэш + Реакции = 10 SSD nVME дисков по 4 Тб = **10 хостов**, если учитывать снижение iops нагрузки, то достаточно **5 хостов**

Для хранения холодных данных фото нам придется использовать RAID, чтобы не увеличивать радикально число хостов:
- 7 стоек по 60 дисков = 420 HDD дисков - т.е 7 хостов + 7 хостов на репликацию итого = **14 хостов**

В горячих данных фото нас больше ограничивает не объем, а throughput и iops, распределив данные по нескольким хостам мы снизим эту нагрузку, в итоге получим следующее:
- (2 * 40 SSD дисков по 100 Тб / 7) / 2 = 12 SSD дисков по 100 Тб распределенных по **7 хостам**, т.е по 2 на хост. Другой вариант, чтобы уменьшить хосты - так же использовать RAID
